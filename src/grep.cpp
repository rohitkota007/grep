...
void helpCheck(char *argv[]) {
	if (argv[1] == std::string("-h") || argv[1] == std::string("--help") || argv[1] == std::string("-help")) {
		std::cout << "\ngrep - Parallel grep by Ryan Chui (2017)\n" << std::endl;
		std::cout << "    grep is a custom multithreaded c++ implementation of grep to search multi gigabyte files, datasets, and\n";
		std::cout << "    directories developed at the National Center for Supercomputing Applications.\n" << std::endl;
		std::cout << "    Usage:\n";
		std::cout << "    grep [-A <#>|-f <file>|-r|-v|-V|-w] <search term>\n" << std::endl;
		std::cout << "    Modes:\n";
		std::cout << "    -A    After Context         grep will grab a number of lines after the line containing the\n";
		...
		std::cout << "    -f    Single File Search    Signals grep to only search the <file> for the <search term>. If -f is not\n";
		std::cout << "                                used, grep will search the entire directory from where grep is called from.\n" << std::endl;
		...
		std::cout << "    -w    File Parallelism      Signals grep to perform single-threaded searches of multiple files. Default\n";
...
void getSettings(int argc, char *argv[], Settings *instance) {
	...
	while (!settings.empty()) {
		...
		if (arg == "-r") {
			...
			if (!settings.empty()) {
				...
				std::cout << "ERROR: The path to the file was not given. \"grep -h\" for help." << std::endl;
				exit(0);
			}
			...
			if (!settings.empty()) {
				...
				std::cout << "ERROR: The number of after context lines was not given. \"grep -h\" for help." << std::endl;
				exit(0);
			}
			if (arg.compare(0, 1, "-") == 0) {
				std::cout << "ERROR: The number of after context lines was not given. \"grep -h\" for help." << std::endl;
			...
			if (settings.size() > 1) {
				std::cout << "ERROR: grep was called incorrectly. \"grep -h\" for command syntax." << std::endl;
				exit(0);
			}
			(*instance).term = arg;
		}
	...
	if ((*instance).term == "") {
		std::cout << "ERROR: Search term not given. \"grep -h\" for help." << std::endl;
		exit(0);
	}
}
